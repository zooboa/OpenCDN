...
function query_domain() {
	...
	$.post(basePath+'index.php/domain/query',{},
		function(data){
...
function append_domain() {
	...
	$.post(basePath+'index.php/domain/append', 
...
function remove_domain(domain_id,domainName) {
	if(confirm("确认删除（"+domainName+"）吗？")) {
		...
		$.post(basePath+'index.php/domain/delete',
			{domain_id:domain_id},
			function(data){
				if (data.result == 0) {
					alert(data.error);
...
function remove_url(domain_id,domainName){
		...
		$.get(basePath+'index.php/remove',{domainName:domainName});
